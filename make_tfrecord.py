...
def all_dirs(file_dir):
    dirs = []
    sorted_file_dir = sorted(os.listdir(file_dir))
    for im_name in sorted_file_dir:
...
def image_to_tfexample(label,real):
    return tf.train.Example(features=tf.train.Features(feature={
                'Label':bytes_feature(label),
                'Real':bytes_feature(real)}))

def _convert_dataset(split_name, filenames, imgnames, TFRECORD_DIR):
    ...
    with tf.Session() as sess:
        ...
        with tf.python_io.TFRecordWriter(output_filename) as tfrecord_writer:
            for i, label_name in enumerate(filenames):
                ...
                example = image_to_tfexample(label_data,real_data)
                tfrecord_writer.write(example.SerializeToString())
                print('procesed image:', i)

def main(args):
    print(args.file_label_dir)
    filenames = all_dirs(args.file_label_dir)
    imgnames = all_dirs(args.file_img_dir)
    training_filenames = shuffle(filenames,args.epoch)
    training_imgnames = shuffle(imgnames,args.epoch)
    _convert_dataset(args.phase, training_filenames, training_imgnames, args.TFRECORD_DIR)
    return True
...
def parse_arguments(argv):
    ...
    parser.add_argument('--file_img_dir', type=str, help='Train or img dir.', default="./data/train/label")
